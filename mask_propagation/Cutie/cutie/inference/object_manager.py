...
class ObjectManager:
    ...
    # My variant
    def purge_selected_objects(self, obj_ids_to_remove_as_tmp_id: List[int]) -> (bool, List[int], List[int]): # (bool, List[int], List[int]):
        obj_id_to_be_deleted = []
        tmp_id_to_be_deleted = []
        tmp_id_to_keep = []
        obj_id_to_keep = []

        # That's the only difference in fact
        obj_ids_to_remove_as_obj_id = [self.tmp_id_to_obj[i].id for i in obj_ids_to_remove_as_tmp_id]

        for obj in self.obj_to_tmp_id:
            if obj.id in obj_ids_to_remove_as_obj_id:
                obj_id_to_be_deleted.append(obj.id)
                tmp_id_to_be_deleted.append(self.obj_to_tmp_id[obj])
            else:
                tmp_id_to_keep.append(self.obj_to_tmp_id[obj])
        ...
        if purge_activated:
            self.delete_objects(obj_id_to_be_deleted)
        return purge_activated, tmp_id_to_keep, obj_id_to_keep
        # return purge_activated, obj_id_to_keep
